...
The within-lease model revoles around 4 institutinal details:

\begin{enumerate}
  \item landlord maintence is costly
...
In equilibrium, predatory landlords are able to suppress tenant complaints, which will extract surplus and distort ex-post quality. Empricically, this will show up as the following comparative statics:

\begin{enumerate}
  \item Retaliatory buildings perform less maintence and this will hold conditional on rent prices 
  \item Retaliation happens on equilibrium for severe enough issues
  \item Worse maintained buildings see more complaints, but conditional on building quality, retaliatory buildings see fewer complaints, as do buildings with higher shares of "precarious" tenants
\end{enumerate}

\section{Environment}

There is a landlord $L$ and a tenant $T$. The interaction lasts one lease period. The landlord chooses whether to perform maintenance; maintenance is \emph{publicly observed}. The goal of the simple model is to show that tenant precarcity is a mechanism by which landlords can shirk on maintenece.
