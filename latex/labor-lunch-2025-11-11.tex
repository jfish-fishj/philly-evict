...
There's been a recent focus on landlord market power with respect to \textbf{posted prices} \parencite{framoutar2024market, calderwang2024algorithmic, watson2024rent, gurun-et-al-2022}
      \begin{itemize}
        \item these papers generally have a traditional quantity restriction mechanism for market power
      \end{itemize}
    \pause
However, many of the most important parts of a landlord-tenant relationship are \textbf{bargained} over
      \begin{itemize}
        \item maintenance requests, lease renewals, etc
...
\begin{frame}{Research Question}
  \begin{center}
    \Large I'm going to try and convince you that landlord-tenant bargaining is an important feature of the low income rental market
  \end{center}
  \pause
  To do that, I'm going to study how landlords choose optimal maintenance levels, and how frictions and threats of eviction can explain lower maintenance and higher profits by high evicting landlords in equilibrium
\end{frame}

%------------------------------------------------
\begin{frame}{This Presentation}
  \begin{itemize}
    \item overview of landlord tenant law and low income rental markets
    \item stylized facts on landlord maintenance and how landlords response to tenant complaints; what I call "equilibrium bargaining"
      \begin{itemize}
        \item high evicting landlords do less maintenance and receive more complaints, and that this holds conditional on property characteristics \textbf{and} contract rents
...
\begin{frame}{Related Literature}
  \begin{itemize}
    ...
    \item market power in rental markets \parencite{framoutar2024market, calderwang2024algorithmic, watson2024rent, gurun-et-al-2022}
  \end{itemize}
\end{frame}
...
\begin{itemize}
    \item Low income landlords generally have \textbf{higher} profit margins \parencite{Desmond_2019,Damen_2025}
     \begin{itemize}
      ...
      \end{itemize}
\end{itemize}
\textbf{equilibrium has to rationalize higher profits, bargaining breakdown, and lower maintenance}
\end{frame}
...
\begin{frame}{Data Continued}
    Putting the data together, I'm going to assemble a quarter-level panel of approximately every rental building in Philadelphia. \\
    \vspace{0.5cm}
    In each quarter, I'm going to observe the number of 311 complaints, evictions, building permits and for a select number of units, contract rent prices.\\
    \vspace{0.5cm}
    I'm going to focus on the 2007-2019 period, as Philadelphia overhauled their eviction system during and after COVID.
\end{frame}
...
\begin{frame}{Landlord Maintenance}
  High filing ($\geq$ 10\% filing rate) landlords receive more complaints, even conditional on rent and property characteristics
\tiny
  \include{tables/complaint_effects}
  ...
  "severe" complaints are those made for heat, fires, drainage, or property maintenance vs non-severe (zoning, licensing issues, and trash collection)
\end{frame}
...
\begin{enumerate}
    ...
    \item Regressions demonstrating that "retaliatory" evictions are more likely to be for less backrent
\end{enumerate}
...
\begin{frame}{Overview of Complaint Process}
  \begin{enumerate}
    ...
    \item if violation is severe enough, property is condemned, and tenant is forced to relocate
  \end{enumerate}
\end{frame}
...
Follow \cite{Dube_2025}, and do a Local Projections Difference in Differences to handle non-absorbing treatment\\
 \begin{itemize}
  ...
  \pause
  \textbf{LP-DiD regression (non-absorbing treatment)}
  \begin{align}
    \Delta^h Y_{it}
  ...
  \small
  \begin{itemize}
    \item \textbf{Sample restriction (clean controls):}
      \begin{itemize}
        \item Treated: $\Delta D_{it} = 1$ and $D_{i,s} = 0$ for $s \in \{t-K,\dots,t-1\}$.
        \item Controls: $D_{i,s} = 0$ for all $s \in \{t-K,\dots,t+h\}$.
      \end{itemize}
    % \item \textbf{Interpretation:} For each $h$, $\beta_h^{LP\text{-}DiD}$ is the average 
    %       \emph{difference-in-differences} in eviction probability between buildings that get a new complaint at $t$
    %       and buildings that remain complaint-free in a window around $t$, measured from $t-1$ to $t+h$.
          % A ``$\sim 200\%$ increase'' corresponds to $\beta_h^{LP\text{-}DiD} \approx 2 \times$ the baseline
          % change in eviction probability for clean controls.
  \end{itemize}
\end{frame}

%------------------------------------------------
\begin{frame}[label=lpdid]{landlord retaliation results}
...
\begin{frame}{landlord retaliation results}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{figs/lp_did_severe_nonsevere.png}
    \caption{Local Projections of Eviction Probability Around Complaints}
  \end{figure}
  \hyperlink{dlags}{\beamergotobutton{Distributed lag results}}
\end{frame}

\begin{frame}{Landlord Changes to Allowed Backrent}
Landlords generally evict tenants who are persistently behind on rent, however, they generally wait until the tenant has accrued ~2 months back rent.\\
\pause
Intuitively, one way landlords can "retaliate" is by giving tenants who complain zero leniency on rent.\\
\pause
One way to look at this is at the percent of cases in a quarter that are filed for one or fewer month's back rent as a function of the whether someone had filed a L\&I complaint

\begin{align}
    Y_{it}
      &= \sum_{\ell=0}^{4} \theta_\ell \,\text{SevereComplaint}_{i,t-\ell}
       + \alpha_i + \lambda_t + \varepsilon_{it},
  \end{align}
\begin{itemize}
    \item \(Y_{it}\) is the percent of cases filed for $\leq$ 1 month back rent,
...
\begin{frame}{Landlord Changes to Allowed Backrent}
    Following a complaint, landlords file a higher share of cases for one month or less back rent
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{figs/grace_plt.png}
    \caption{Distributed Lag of Percent No Grace Around Complaints}
  \end{figure}
\end{frame}
...
\begin{enumerate}
    ...
    \item Have higher profits
\end{enumerate}

\parencite{Bebchuk_1984}-style model is going to rationalize 1) and 3). \\
\vspace{0.5cm}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Lower amounts of maintenance is consistent with data
        \item Equal acceptance probabilities is falsified. Suggests need for reputation effects and a dynamic landlord problem
    \end{itemize}
...
\begin{itemize}
    ...
    \item Perform maintenance to encourage tenants to resign lease
\end{itemize}
...
\begin{frame}[label=dlags]{Distributed Lag Results}
  ...
  \begin{align}
    Y_{it}
      &= \sum_{\ell=0}^{4} \theta_\ell \,\text{Complaint}_{i,t-\ell}
       + \alpha_i + \lambda_t + \varepsilon_{it},
  \end{align}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/lpm_retaliatory_any_complaint.png}
    \caption{Distributed Lag Effects of Complaints on Eviction Filings}
  \end{figure}
  \hyperlink{lpdid}{\beamergotobutton{LPDiD}}
\end{frame}
