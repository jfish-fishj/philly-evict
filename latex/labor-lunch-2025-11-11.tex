  ...
  \begin{itemize}
    \item there's been a recent focus on landlord market power on \textbf{*posted prices*} % <put some cites>
      \begin{itemize}
        ...
        \item scale and concentration matter for rationalizing these effects
      \end{itemize}
    \pause
    \item however, many of the most important parts of a landlord-tenant relationship are \textbf{*bargained*} over
      \begin{itemize}
        ...
        \item ex ante reasons to think this bargaining process is particularly important for explaining the low income rental market
      \end{itemize}
  \end{itemize}
...
\begin{frame}{Research Question}
  \begin{center}
    \Large im going to try and convince you that bargaining is an important feature of the rental market
  \end{center}
  % Suggestion: consider “I'm going to try to convince you that bargaining is an important feature of the rental market.”
\end{frame}
...
\begin{frame}{This Presentation}
  \begin{itemize}
    ...
    \item stylized facts on equilibrium bargaining
      \begin{itemize}
        \item high evicting landlords do less maintence, conditional on property characteristics \textbf{*and*} contract rents
        \item landlords retaliate
      \end{itemize}
    \pause
    \item stylized bargaining model to rationalize facts
  \end{itemize}
\end{frame}
...
\begin{frame}{Related Literature}
  \begin{itemize}
    \item plea bargains
    \item divorce law
    \item bargaining with asymetric information
  \end{itemize}
  % Suggestion: you could later add one explicit example/cite under each bullet to orient the audience.
\end{frame}

%------------------------------------------------
\begin{frame}{Overview of Low Income Rental Market}
  \begin{itemize}
    \item tenant default is common; % <put that humphries cite about here>
      \begin{itemize}
        \item most cases involve non-payment of rent; median case is for \(\sim\)2 months back rent
      ...
      \end{itemize}
    \pause
    \item low income landlords generally have \textbf{*higher*} profit margins
      \begin{itemize}
...
\begin{frame}{Data}
  \begin{itemize}
    \item Administrative data on evictions, complaints, and permits.
    \item Property characteristics and rent prices.
    \item Linked to tenants/landlords where possible.
  \end{itemize}
\end{frame}
...
\begin{frame}{Data (Cont.)}
  \begin{itemize}
    \item Construction of the building-by-year-quarter panel.
    \item Key variables:
      \begin{itemize}
        \item eviction filings,
        \item maintenance permits,
        \item complaints,
        \item contract rent.
      \end{itemize}
  \end{itemize}
...
------------------------------------------------
\begin{frame}{Summary Statistics on Evictions}
  \begin{table}[H]
    \centering
    \begin{tabular}{lccc}
      \toprule
      ...
      \bottomrule
    \end{tabular}
    \caption{Summary Statistics on Evictions}
  \end{table}
\end{frame}
...
%------------------------------------------------
\begin{frame}{Summary Table on Rent Prices}
  \begin{table}[H]
    \centering
    \begin{tabular}{lccc}
      \toprule
      ...
      % Fill in with actual variables
      Contract rent & & & \\
      Implied rent per square foot & & & \\
      % ...
      \bottomrule
    \end{tabular}
    \caption{Summary Statistics on Rent Prices}
  \end{table}
\end{frame}
...
\begin{frame}{landlord maintenance}
  ...
  \medskip
  \textcolor{maintGreen}{Baseline specification:}
  \begin{align}
    \text{count permits}_{it}
      &= \beta_0
       + \beta_1 \,\text{high\_evicting}_i
       + \beta_2 \,\text{Property Characteristics}_{it}
       + \beta_3 \,\text{rent price}_{it}
       + \varepsilon_{it}.
  \end{align}

  \medskip
  \begin{itemize}
    \item \(\beta_1 < 0\) captures that high evicting landlords do less maintenance conditional on observables.
  \end{itemize}
  ...
  \begin{table}[H]
    ...
    \begin{tabular}{lccc}
      ...
      \bottomrule
    \end{tabular}
    \caption{Regressions of Maintenance Permits on High-Evicting Indicator}
  \end{table}
...
\begin{frame}{overview of complaint process}
  \begin{enumerate}
    ...
    \item if landlord does not fix complaint, tenant can file a 311 complaint with the Licenses and Inspections Department
      \begin{itemize}
        \item while complaint has been lodged, tenant is allowed to withold rent \textbf{*by putting it in an escrow account*}
