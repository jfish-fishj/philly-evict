...
library(bit64)
library(glue)
indir = "~/Desktop/data/altos/cities/"
cities = list.files(indir, pattern = "_metro_all_years.csv") %>%
  str_remove("_metro_all_years.csv")
cleaned_cities = list.files(indir, pattern = "_metro_all_years_clean_addys.csv") %>%
  str_remove("_metro_all_years_clean_addys.csv")
...
for(
  ...
  outfile = glue("~/Desktop/data/altos/cities/{metro}_metro_all_years_clean_addys.csv")
  altos = fread(glue("~/Desktop/data/altos/cities/{metro}_metro_all_years.csv"))
#### functions ####
...
altos_adds_sample = altos_adds_sample %>%
  mutate(
    pm.address = coalesce(pm.address, pm.streetSuf) %>%
      str_to_lower() ,
