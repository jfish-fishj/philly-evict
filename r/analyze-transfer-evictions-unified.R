...
if (freq == "annual" && !is.null(full_panel_main)) {
  ...
  baseline_poisson_specs <- list(
    ...
    list(label = "2plus", sample_filter = quote(total_units >= 2L), header = "2+ unit buildings"),
    list(label = "5plus", sample_filter = quote(total_units >= 5L), header = "5+ unit buildings"),
    list(label = "2plus_rental_pre", sample_filter = quote(total_units >= 2L & observed_rental_stock == TRUE), header = "2+ units, observed rental stock"),
    list(label = "5plus_rental_pre", sample_filter = quote(total_units >= 5L & observed_rental_stock == TRUE), header = "5+ units, observed rental stock")
  )
  run_baseline_poisson_override <- function() {
    baseline_override_coefs <- list()
    baseline_override_fits <- list()
    for (bs in baseline_poisson_specs) {
      .panel <- subset_panel_main(full_panel_main, sample_filter = bs$sample_filter, outcome_col = "num_filings")
      .res <- run_named_fullpanel_poisson(.panel, bs$label)
      if (!is.null(.res)) {
        baseline_override_coefs[[length(baseline_override_coefs) + 1L]] <- .res$coefs
        baseline_override_fits[[bs$header]] <- .res$fit
      }
    ...
    baseline_all <- rbindlist(baseline_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(baseline_all, out_path("baseline_poisson_coefs.csv"))
    plot_event_coefs(
      baseline_all,
      unique(baseline_all$spec),
      paste0("Event Study: Filing Counts with Unit Offset Around Transfer (", fp$label, ")"),
      fig_path("baseline_poisson.png"),
      facet = TRUE,
      y_label = "Poisson coefficient: log expected filings per unit-year"
    )
    etable(
      baseline_override_fits,
      file = out_path("baseline_poisson_etable.tex"),
      style.tex = style.tex("aer"),
      headers = list("Sample" = names(baseline_override_fits))
    )
    baseline_all
  }
  baseline_poisson_all <- run_baseline_poisson_override()

  # --- Buyer type ---
  buyer_override_specs <- list(
    list(label = "corporate", treated_filter = quote(is_corp == TRUE), sample_filter = NULL, header = "Corporate"),
    list(label = "individual", treated_filter = quote(is_corp == FALSE), sample_filter = NULL, header = "Individual"),
    list(label = "corporate_large", treated_filter = quote(is_corp == TRUE), sample_filter = quote(total_units >= 5L), header = "Corp (5+)"),
    list(label = "individual_large", treated_filter = quote(is_corp == FALSE), sample_filter = quote(total_units >= 5L), header = "Indiv (5+)")
  )
  buyer_2plus_specs <- list(
    list(label = "corporate_2plus", treated_filter = quote(is_corp == TRUE), sample_filter = quote(total_units >= 2L), header = "Corporate 2+"),
    list(label = "individual_2plus", treated_filter = quote(is_corp == FALSE), sample_filter = quote(total_units >= 2L), header = "Individual 2+")
  )
  run_buyer_override <- function(lhs, out_stem, plot_title, y_label,
                                 buyer_specs = buyer_override_specs,
                                 write_table = TRUE) {
    buyer_override_coefs <- list()
    buyer_override_fits <- list()
    for (buyer_spec in buyer_specs) {
      .panel <- subset_panel_main(
        full_panel_main,
        sample_filter = buyer_spec$sample_filter,
        treated_filter = buyer_spec$treated_filter,
        outcome_col = lhs
      )
      .res <- run_named_fullpanel(.panel, buyer_spec$label, lhs = lhs)
      if (!is.null(.res)) {
        buyer_override_coefs[[length(buyer_override_coefs) + 1L]] <- .res$coefs
        buyer_override_fits[[buyer_spec$header]] <- .res$fit
      }
    }
    if (length(buyer_override_coefs) == 0L) {
      return(invisible(NULL))
    }
    buyer_all <- rbindlist(buyer_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(buyer_all, out_path(paste0(out_stem, "_coefs.csv")))
    plot_event_coefs(
      buyer_all,
      unique(buyer_all$spec),
      paste0(plot_title, " (", fp$label, ")"),
      fig_path(paste0(gsub("^buyer_type", "by_buyer", out_stem), ".png")),
      facet = TRUE,
      y_label = y_label
    )
    if (write_table) {
      etable(
        ...
        file = out_path(paste0(out_stem, "_etable.tex")),
        style.tex = style.tex("aer"),
        headers = list("Buyer type" = names(buyer_override_fits))
      )
    }
    buyer_all
  }
  buyer_all <- run_buyer_override(
    lhs = "filing_rate",
    out_stem = "buyer_type",
    plot_title = "Event Study by Buyer Type",
    y_label = "Filing Rate (per unit-year)"
  )
  buyer_2plus_all <- run_buyer_override(
    lhs = "filing_rate",
    out_stem = "buyer_type_2plus",
    plot_title = "Event Study by Buyer Type (2+ Units)",
    y_label = "Filing Rate (per unit-year)",
    buyer_specs = buyer_2plus_specs,
    write_table = FALSE
  )
  run_buyer_override_poisson <- function(out_stem, plot_title, buyer_specs = buyer_override_specs,
                                         write_table = TRUE) {
    buyer_override_coefs <- list()
    buyer_override_fits <- list()
    for (buyer_spec in buyer_specs) {
      .panel <- subset_panel_main(
        ...
        sample_filter = buyer_spec$sample_filter,
        treated_filter = buyer_spec$treated_filter,
        outcome_col = "num_filings"
      )
      .res <- run_named_fullpanel_poisson(.panel, buyer_spec$label)
      if (!is.null(.res)) {
        buyer_override_coefs[[length(buyer_override_coefs) + 1L]] <- .res$coefs
        buyer_override_fits[[buyer_spec$header]] <- .res$fit
      }
  ...
  buyer_poisson_all <- run_buyer_override_poisson(
    out_stem = "buyer_type_poisson",
    plot_title = "Event Study by Buyer Type: Poisson with Unit Offset"
  )
  buyer_poisson_2plus_all <- run_buyer_override_poisson(
    out_stem = "buyer_type_poisson_2plus",
    plot_title = "Event Study by Buyer Type: Poisson with Unit Offset (2+ Units)",
    buyer_specs = buyer_2plus_specs,
    write_table = FALSE
  )

  # --- Building size ---
  size_override_coefs <- list()
  size_override_fits <- list()
  for (sb in all_size_bins) {
    .panel <- subset_panel_main(full_panel_main,
                                sample_filter = substitute(size_bin == SB, list(SB = sb)),
                                outcome_col = "filing_rate")
    .res <- run_named_fullpanel(.panel, paste0("size_", sb))
    if (!is.null(.res)) {
      size_override_coefs[[length(size_override_coefs) + 1L]] <- .res$coefs
      size_override_fits[[sb]] <- .res$fit
    }
  }
  if (length(size_override_coefs) > 0L) {
    size_all <- rbindlist(size_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(size_all, out_path("size_coefs.csv"))
    plot_event_coefs(size_all, unique(size_all$spec),
                     paste0("Event Study by Building Size (", fp$label, ")"),
                     fig_path("by_size.png"), facet = TRUE)
    etable(size_override_fits,
           file = out_path("size_etable.tex"),
           style.tex = style.tex("aer"),
           headers = list("Building size" = names(size_override_fits)))
  }

  corp_size_override <- list()
  for (sb in all_size_bins) {
    for (buyer_label in c("corp", "indiv")) {
      .panel <- subset_panel_main(
        full_panel_main,
        sample_filter = substitute(size_bin == SB, list(SB = sb)),
        treated_filter = if (buyer_label == "corp") quote(is_corp == TRUE) else quote(is_corp == FALSE),
        outcome_col = "filing_rate"
      )
      .spec <- paste0(buyer_label, "_", sb)
      .res <- run_named_fullpanel(.panel, .spec)
      if (!is.null(.res)) corp_size_override[[length(corp_size_override) + 1L]] <- .res$coefs
    }
  }
  if (length(corp_size_override) > 0L) fwrite(rbindlist(corp_size_override), out_path("corp_x_size_coefs.csv"))

  # --- Acquirer filing rate (For-profit corp / Person / Trust acquirers only) ---
  acq_override_coefs <- list()
  acq_override_fits <- list()
  for (ab in acq_bins_ordered) {
    .panel <- subset_panel_main(full_panel_filer,
                                treated_filter = substitute(acq_filer_bin == AB, list(AB = ab)),
                                outcome_col = "filing_rate")
    .spec <- paste0("full_", acq_bin_suffix(ab))
    .res <- run_named_fullpanel(.panel, .spec)
    if (!is.null(.res)) {
      acq_override_coefs[[length(acq_override_coefs) + 1L]] <- .res$coefs
      acq_override_fits[[ab]] <- .res$fit
    }
  }
  if (length(acq_override_coefs) > 0L) {
    acq_all <- rbindlist(acq_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(acq_all, out_path("acq_filer_coefs.csv"))
    plot_event_coefs(acq_all, unique(acq_all$spec),
                     paste0("Event Study by Acquirer Filing Rate (", fp$label, ")"),
                     fig_path("by_acq_filer.png"), facet = TRUE)
    etable(acq_override_fits,
           file = out_path("acq_filer_etable.tex"),
           style.tex = style.tex("aer"),
           headers = list("Acquirer type" = names(acq_override_fits)))
  }

  acq_2plus_override_coefs <- list()
  for (ab in acq_bins_ordered) {
    .panel <- subset_panel_main(full_panel_filer,
                                sample_filter = quote(total_units >= 2L),
                                treated_filter = substitute(acq_filer_bin == AB, list(AB = ab)),
                                outcome_col = "filing_rate")
    .spec <- paste0("2plus_", acq_bin_suffix(ab))
    .res <- run_named_fullpanel(.panel, .spec)
    if (!is.null(.res)) {
      acq_2plus_override_coefs[[length(acq_2plus_override_coefs) + 1L]] <- .res$coefs
    }
  }
  if (length(acq_2plus_override_coefs) > 0L) {
    acq_2plus_all <- rbindlist(acq_2plus_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(acq_2plus_all, out_path("acq_filer_2plus_coefs.csv"))
    plot_event_coefs(acq_2plus_all, unique(acq_2plus_all$spec),
                     paste0("Event Study by Acquirer Filing Rate (", fp$label, "; 2+ Units)"),
                     fig_path("by_acq_filer_2plus.png"), facet = TRUE)
  }
  run_acq_override_poisson <- function(out_stem, plot_title, sample_filter = NULL, write_table = TRUE) {
    acq_override_coefs <- list()
    acq_override_fits <- list()
    for (ab in acq_bins_ordered) {
      .panel <- subset_panel_main(
      ...
      .spec <- paste0(if (is.null(sample_filter)) "full_" else "2plus_", acq_bin_suffix(ab))
      .res <- run_named_fullpanel_poisson(.panel, .spec)
      if (!is.null(.res)) {
        acq_override_coefs[[length(acq_override_coefs) + 1L]] <- .res$coefs
        acq_override_fits[[ab]] <- .res$fit
      }
    ...
    acq_all <- rbindlist(acq_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(acq_all, out_path(paste0(out_stem, "_coefs.csv")))
    plot_event_coefs(
      acq_all,
      unique(acq_all$spec),
      paste0(plot_title, " (", fp$label, ")"),
      fig_path(paste0(gsub("^acq_filer", "by_acq_filer", out_stem), ".png")),
      facet = TRUE,
      y_label = "Poisson coefficient: log expected filings per unit-year"
    )
    if (write_table) {
  ...
  acq_poisson_all <- run_acq_override_poisson(
    out_stem = "acq_filer_poisson",
    plot_title = "Event Study by Acquirer Filing Rate: Poisson with Unit Offset"
  )
  acq_poisson_2plus_all <- run_acq_override_poisson(
    out_stem = "acq_filer_poisson_2plus",
    plot_title = "Event Study by Acquirer Filing Rate: Poisson with Unit Offset (2+ Units)",
    sample_filter = quote(total_units >= 2L),
    write_table = FALSE
  )
  ...
  raw_acq_dt <- full_panel_filer[
    ...
    .(
      mean_filing_rate    = weighted.mean(filing_rate, total_units, na.rm = TRUE),
      n_pids              = uniqueN(PID),
      n_obs               = .N,
      pct_zero_filings    = mean(num_filings == 0L, na.rm = TRUE),
      mean_units          = mean(total_units, na.rm = TRUE)
    ),
    by = .(acq_filer_bin, event_time)
  ]
  setorder(raw_acq_dt, acq_filer_bin, event_time)
  fwrite(raw_acq_dt, out_path("acq_filer_raw_means_by_et.csv"))

  # Cohort breakdown for high-filer bin: mean filing rate by (event_time, transfer_year cohort)
  cohort_hi_dt <- full_panel_filer[
    acq_filer_bin == hi_bin &
    !is.na(event_time) & event_time %in% fp$et_range,
    .(
      mean_filing_rate = weighted.mean(filing_rate, total_units, na.rm = TRUE),
      n_pids           = uniqueN(PID),
      pct_zero         = mean(num_filings == 0L, na.rm = TRUE)
    ),
    by = .(transfer_year, event_time)
  ]
  setorder(cohort_hi_dt, transfer_year, event_time)
  fwrite(cohort_hi_dt, out_path("acq_filer_hi_cohort_raw_means.csv"))

  # Calendar-year view: mean filing rate by calendar year for high-filer treated PIDs
  cal_hi_dt <- full_panel_filer[
    acq_filer_bin == hi_bin & !is.na(event_time) & event_time %in% fp$et_range,
    .(
      mean_filing_rate = weighted.mean(filing_rate, total_units, na.rm = TRUE),
      n_pids           = uniqueN(PID),
      pct_zero         = mean(num_filings == 0L, na.rm = TRUE)
  ...
  setorder(cal_hi_dt, event_time, year)
  fwrite(cal_hi_dt, out_path("acq_filer_hi_cal_year_raw_means.csv"))
  logf("  Wrote acq_filer raw-means diagnostics (by ET, cohort, calendar year)",
       log_file = log_file)

  # --- Portfolio size ---
  portfolio_override_coefs <- list()
  portfolio_override_fits <- list()
  for (pb in portfolio_bins) {
    .panel <- subset_panel_main(full_panel_filer,
                                treated_filter = substitute(portfolio_bin == PB, list(PB = pb)),
                                outcome_col = "filing_rate")
    .spec <- paste0("portfolio_", pb)
    .res <- run_named_fullpanel(.panel, .spec)
    if (!is.null(.res)) {
      portfolio_override_coefs[[length(portfolio_override_coefs) + 1L]] <- .res$coefs
      portfolio_override_fits[[pb]] <- .res$fit
    }
  }
  if (length(portfolio_override_coefs) > 0L) {
    portfolio_all <- rbindlist(portfolio_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(portfolio_all, out_path("portfolio_coefs.csv"))
    plot_event_coefs(portfolio_all, unique(portfolio_all$spec),
                     paste0("Event Study by Portfolio Size (", fp$label, ")"),
                     fig_path("by_portfolio.png"), facet = TRUE)
    etable(portfolio_override_fits,
           file = out_path("portfolio_etable.tex"),
           style.tex = style.tex("aer"),
           headers = list("Portfolio size" = names(portfolio_override_fits)))
  }

  portfolio_2plus_override_coefs <- list()
  for (pb in portfolio_bins) {
    .panel <- subset_panel_main(full_panel_filer,
                                sample_filter = quote(total_units >= 2L),
                                treated_filter = substitute(portfolio_bin == PB, list(PB = pb)),
                                outcome_col = "filing_rate")
    .spec <- paste0("portfolio_2plus_", pb)
    .res <- run_named_fullpanel(.panel, .spec)
    if (!is.null(.res)) {
      portfolio_2plus_override_coefs[[length(portfolio_2plus_override_coefs) + 1L]] <- .res$coefs
    }
  }
  if (length(portfolio_2plus_override_coefs) > 0L) {
    portfolio_2plus_all <- rbindlist(portfolio_2plus_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(portfolio_2plus_all, out_path("portfolio_2plus_coefs.csv"))
    plot_event_coefs(portfolio_2plus_all, unique(portfolio_2plus_all$spec),
                     paste0("Event Study by Portfolio Size (", fp$label, "; 2+ Units)"),
                     fig_path("by_portfolio_2plus.png"), facet = TRUE)
  }
  run_portfolio_override_poisson <- function(out_stem, plot_title, sample_filter = NULL, write_table = TRUE) {
    portfolio_override_coefs <- list()
    portfolio_override_fits <- list()
    for (pb in portfolio_bins) {
      .panel <- subset_panel_main(
      ...
      .spec <- paste0("portfolio_", if (!is.null(sample_filter)) "2plus_" else "", pb)
      .res <- run_named_fullpanel_poisson(.panel, .spec)
      if (!is.null(.res)) {
        portfolio_override_coefs[[length(portfolio_override_coefs) + 1L]] <- .res$coefs
        portfolio_override_fits[[pb]] <- .res$fit
    ...
    portfolio_all <- rbindlist(portfolio_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(portfolio_all, out_path(paste0(out_stem, "_coefs.csv")))
    plot_event_coefs(
      portfolio_all,
      unique(portfolio_all$spec),
      paste0(plot_title, " (", fp$label, ")"),
      fig_path(paste0(gsub("^portfolio", "by_portfolio", out_stem), ".png")),
      facet = TRUE,
      y_label = "Poisson coefficient: log expected filings per unit-year"
    )
    if (write_table) {
      etable(
        portfolio_override_fits,
        file = out_path(paste0(out_stem, "_etable.tex")),
        style.tex = style.tex("aer"),
        headers = list("Portfolio size" = names(portfolio_override_fits))
      )
  ...
  portfolio_poisson_all <- run_portfolio_override_poisson(
    out_stem = "portfolio_poisson",
    plot_title = "Event Study by Portfolio Size: Poisson with Unit Offset"
  )
  portfolio_poisson_2plus_all <- run_portfolio_override_poisson(
    out_stem = "portfolio_poisson_2plus",
    plot_title = "Event Study by Portfolio Size: Poisson with Unit Offset (2+ Units)",
    sample_filter = quote(total_units >= 2L),
    write_table = FALSE
  )

  # --- Corporate x filer ---
  corp_filer_override <- list()
  for (ab in acq_bins_ordered) {
    for (corp_flag in c(TRUE, FALSE)) {
      .panel <- subset_panel_main(
        full_panel_filer,
        treated_filter = substitute(acq_filer_bin == AB & is_corp == CF, list(AB = ab, CF = corp_flag)),
        outcome_col = "filing_rate"
      )
      .spec <- paste0(if (corp_flag) "corp_" else "indiv_",
                      acq_bin_suffix(ab))
      .res <- run_named_fullpanel(.panel, .spec)
      if (!is.null(.res)) corp_filer_override[[length(corp_filer_override) + 1L]] <- .res$coefs
    }
  }
  if (length(corp_filer_override) > 0L) {
    corp_filer_all <- rbindlist(corp_filer_override, use.names = TRUE, fill = TRUE)
    fwrite(corp_filer_all, out_path("corp_x_filer_coefs.csv"))
    plot_event_coefs(corp_filer_all, unique(corp_filer_all$spec),
                     paste0("Event Study: Corporate x Acquirer Filer Bin (", fp$label, ")"),
                     fig_path("corp_x_filer.png"), facet = TRUE)
  ...
  port_units_spec_labels <- c("port_units_0_solo", "port_units_1_9",
                              "port_units_10_49", "port_units_50plus")
  pu_override_coefs <- list()
  pu_override_fits <- list()
  for (i in seq_along(port_units_bins)) {
    pub <- port_units_bins[i]
    .panel <- subset_panel_main(full_panel_filer,
                                treated_filter = substitute(portfolio_units_bin == PUB, list(PUB = pub)),
                                outcome_col = "filing_rate")
    .res <- run_named_fullpanel(.panel, port_units_spec_labels[i])
    if (!is.null(.res)) {
      pu_override_coefs[[length(pu_override_coefs) + 1L]] <- .res$coefs
      pu_override_fits[[pub]] <- .res$fit
    }
  }
  if (length(pu_override_coefs) > 0L) {
    pu_all <- rbindlist(pu_override_coefs, use.names = TRUE, fill = TRUE)
    fwrite(pu_all, out_path("portfolio_units_coefs.csv"))
    plot_event_coefs(pu_all, unique(pu_all$spec),
                     paste0("Event Study by Portfolio Units (net of acquiring building, ", fp$label, ")"),
                     fig_path("by_portfolio_units.png"), facet = TRUE)
    etable(pu_override_fits,
           file = out_path("portfolio_units_etable.tex"),
           style.tex = style.tex("aer"),
           headers = list("Portfolio units (net)" = names(pu_override_fits)))
  }

  # --------------------------------------------------------
  # Neighborhood gentrification status heterogeneity (annual full-panel design)
  # Single interacted model: i(event_time, nhood_gent, ref=c(-1,"Not gentrifiable"))
  # Coefficients = deviation from "Not gentrifiable" baseline path.
  # --------------------------------------------------------
  logf("--- Annual override: nhood gentrification event study ---", log_file = log_file)
  if (!all(is.na(full_panel_main$nhood_gent))) {
    # Use -3:3 to match stacked DiD window (stacked DiD used k ∈ {-3,...,+3})
    GENT_ET_RANGE <- -3L:3L
    et_dummies_gent <- paste0("et_", ifelse(setdiff(GENT_ET_RANGE, ET_REF) < 0,
                                             paste0("m", abs(setdiff(GENT_ET_RANGE, ET_REF))),
                                             setdiff(GENT_ET_RANGE, ET_REF)))

    gent_panel <- subset_panel_main(full_panel_main,
                                    sample_filter = quote(total_units >= 2L),
                                    outcome_col   = "filing_rate")
    # Clip to -3:3 window (controls have NA event_time — keep them)
    gent_panel <- gent_panel[is.na(event_time) | event_time %in% GENT_ET_RANGE]
    gent_panel <- gent_panel[!is.na(nhood_gent)]

    # Interacted model: separate paths per gent level, "Not gentrifiable" as base
    fit_gent_interact_main <- tryCatch(feols(
      filing_rate ~ i(event_time, nhood_gent,
                      ref = ET_REF, ref2 = "Not gentrifiable") |
        PID + year,
      data    = gent_panel,
      cluster = ~PID,
      weights = ~total_units
    ), error = function(e) {
      logf("  WARNING: interacted gent model failed: ", conditionMessage(e), log_file = log_file)
      NULL
    })

    if (!is.null(fit_gent_interact_main)) {
      saveRDS(fit_gent_interact_main, out_path("nhood_gent_interact_fit.rds"))
      logf("  nhood_gent interacted model: ", length(coef(fit_gent_interact_main)),
           " coefficients", log_file = log_file)
      png(fig_path("by_nhood_gent_interact.png"), width = 1400, height = 700, res = 150)
      coefplot(fit_gent_interact_main,
               main = paste0("Filing rate × nhood gentrification status (", fp$label, ")"),
               sub  = "Coefficients = deviation from Not gentrifiable × ref-period baseline")
      dev.off()
      logf("  Wrote by_nhood_gent_interact.png", log_file = log_file)
    }

    # Separate-sample specs (for overlay plot + etable)
    GENT_LEVELS_ANN <- c("Gentrified", "Did not gentrify", "Not gentrifiable")
    GENT_PALETTE_ANN <- c(
      "Gentrified"        = "#D1495B",
      "Did not gentrify"  = "#E9C46A",
      "Not gentrifiable"  = "#2A9D8F"
    ...
    gent_ann_coefs <- list()
    gent_ann_fits  <- list()
    for (gs in GENT_LEVELS_ANN) {
      .panel <- subset_panel_main(full_panel_main,
                                  sample_filter = substitute(
                                    total_units >= 2L & nhood_gent == GS, list(GS = gs)),
                                  outcome_col   = "filing_rate")
      .panel <- .panel[is.na(event_time) | event_time %in% GENT_ET_RANGE]
      if (.panel[!is.na(transfer_year), uniqueN(PID)] < 10L) next
      .res <- run_named_fullpanel(.panel, paste0("nhood_", gsub("[^a-zA-Z]", "_", tolower(gs))),
                                  dummies = et_dummies_gent)
      if (!is.null(.res)) {
        gent_ann_coefs[[length(gent_ann_coefs) + 1L]] <- .res$coefs
        gent_ann_fits[[gs]] <- .res$fit
      }
    }
    if (length(gent_ann_coefs) > 0L) {
      gent_ann_all <- rbindlist(gent_ann_coefs, use.names = TRUE, fill = TRUE)
      fwrite(gent_ann_all, out_path("nhood_gent_coefs.csv"))
      plot_event_coefs(gent_ann_all, unique(gent_ann_all$spec),
                       paste0("Event Study by Nhood Gentrification Status (", fp$label, ")"),
                       fig_path("by_nhood_gent.png"),
                       facet = FALSE)
      if (length(gent_ann_fits) > 0L) {
        etable(gent_ann_fits,
               file      = out_path("nhood_gent_etable.tex"),
               style.tex = style.tex("aer"),
               headers   = list("Nhood trajectory" = names(gent_ann_fits)))
      }
      logf("  Wrote nhood_gent_coefs.csv, by_nhood_gent.png, nhood_gent_etable.tex",
           log_file = log_file)
    }
  } else {
    logf("  Skipping nhood gent event study: nhood_gent not available",
         log_file = log_file)
  }
  ...
  # controls have acq_rate=NA and are dropped from the i() interaction automatically.
  port_acq_panel <- subset_panel_main(
    full_panel_filer,
    sample_filter  = quote(total_units >= 2L),
  ...
  # Unit-weighted mean at the PID level (one obs per acquisition, weighted by acquired building
  # size). Matches regression weights (~total_units) and avoids double-counting panel years.
  port_acq_pid <- unique(port_acq_panel[!is.na(acq_rate)], by = "PID")
  port_acq_mean <- port_acq_pid[, weighted.mean(acq_rate, total_units, na.rm = TRUE)]
  # Treated: demeaned rate. Controls (acq_rate=NA): set to 0 (at the mean) so they are NOT
  # dropped by fixest's NA handling. Controls have post_treat=0 and event_time=NA (all et_
  # dummies=0), so their acq_rate_demean=0 never enters any interaction term — they identify
  # only the year FE and the post_treat main effect, which is correct DiD behavior.
  port_acq_panel[, acq_rate_demean := acq_rate - port_acq_mean]
  port_acq_panel[is.na(acq_rate), acq_rate_demean := 0]
  logf("  Portfolio-acquirer sample (treated+controls, 2+ units): N=", nrow(port_acq_panel),
       ", treated PIDs=", port_acq_panel[!is.na(transfer_year), uniqueN(PID)],
       ", control PIDs=", port_acq_panel[is.na(transfer_year), uniqueN(PID)],
       ", unit-wtd mean acq_rate (treated)=", round(port_acq_mean, 5), log_file = log_file)

  if (nrow(port_acq_panel) >= 100 && port_acq_panel[, uniqueN(PID)] >= 10) {
    # (i) Continuous event-time × acq_rate interaction.
    # Use pre-computed et_ dummies (set to 0 for controls, not NA) rather than i(event_time, ...)
    # which generates NA interaction terms when event_time=NA, dropping all control rows.
    # et_dummies_main excludes the reference period (ET_REF); controls have all et_ = 0, so
    # their acq_rate_demean=0 interaction contributes nothing to the slope — correct DiD behavior.
    cont_int_terms <- paste0(et_dummies_main, ":acq_rate_demean", collapse = " + ")
    cont_fml_str   <- paste0("filing_rate ~ ", cont_int_terms, " | PID + year")
    port_acq_cont  <- port_acq_panel[is.na(event_time) | event_time %in% fp$et_range]
    fit_cont <- feols(
      as.formula(cont_fml_str),
      data = port_acq_cont, cluster = ~PID, weights = ~total_units
    )
    ct_cont <- as.data.table(coeftable(fit_cont), keep.rownames = "term")
    setnames(ct_cont, c("term", "estimate", "std_error", "t_value", "p_value"))
    ct_cont[, spec := "acq_rate_continuous"]
    fwrite(ct_cont, out_path("acq_filer_continuous_coefs.csv"))
    logf("  Wrote acq_filer_continuous_coefs.csv (", nrow(ct_cont), " terms)",
         log_file = log_file)

    # (ii) Pooled post-treatment DiD: post_treat × acq_rate_demean
    # post_treat = 1 for event_time >= 1 (post-transfer), 0 for pre (t < 0) and controls.
    # t=0 (transfer year itself) excluded — ambiguous exposure timing.
    # Never-sold controls: event_time=NA → post_treat=0 (always pre), acq_rate_demean=NA →
    # they enter the year FE but are dropped from the interaction term by fixest.
    port_acq_panel[, post_treat := fcase(
      !is.na(event_time) & event_time >= 1L,  1L,
      !is.na(event_time) & event_time <  0L,  0L,
      is.na(event_time),                      0L,   # never-sold controls: always "pre"
      default = NA_integer_
    )]
    # Restrict to event window for treated (excluding t=0), all years for controls
    port_post_dt <- port_acq_panel[
      !is.na(post_treat) &
      (is.na(event_time) | (event_time %in% fp$et_range & event_time != 0L))
    ]
    logf("  Pooled DiD sample: N=", nrow(port_post_dt),
         ", treated PIDs=", port_post_dt[!is.na(transfer_year), uniqueN(PID)],
         ", control PIDs=", port_post_dt[is.na(transfer_year), uniqueN(PID)],
         log_file = log_file)

    if (nrow(port_post_dt) >= 100 && port_post_dt[, uniqueN(PID)] >= 10) {
      fit_did <- feols(
        filing_rate ~ post_treat + post_treat:acq_rate_demean | PID + year,
        data = port_post_dt, cluster = ~PID, weights = ~total_units
      )
      ct_did <- as.data.table(coeftable(fit_did), keep.rownames = "term")
      setnames(ct_did, c("term", "estimate", "std_error", "t_value", "p_value"))
      ct_did[, spec := "acq_rate_pooled_did"]
      fwrite(ct_did, out_path("acq_filer_pooled_did_coefs.csv"))

      # Extra lines for etable: treated/control PIDs, acq_rate SD
      n_treated_cont <- port_acq_cont[!is.na(transfer_year), uniqueN(PID)]
      n_control_cont <- port_acq_cont[is.na(transfer_year),  uniqueN(PID)]
      n_treated_did  <- port_post_dt[!is.na(transfer_year),  uniqueN(PID)]
      n_control_did  <- port_post_dt[is.na(transfer_year),   uniqueN(PID)]
      sd_acq         <- round(port_acq_pid[, sd(acq_rate)], 4)

      etable(list("Continuous $\\times$ event-time" = fit_cont,
                  "Pooled post DiD"                 = fit_did),
             file      = out_path("acq_filer_continuous_etable.tex"),
             style.tex = style.tex("aer"),
             extralines = list(
               "Treated PIDs"     = list(n_treated_cont, n_treated_did),
               "Control PIDs"     = list(n_control_cont, n_control_did),
               "SD(acq rate)"     = list(sd_acq, sd_acq),
               "Mean(acq rate)"   = list(round(port_acq_mean, 4), round(port_acq_mean, 4))
             ))
      logf("  Wrote acq_filer_pooled_did_coefs.csv + acq_filer_continuous_etable.tex",
           log_file = log_file)
    }
  }

  run_outcome_fullpanel <- function(outcome_col, out_stem, outcome_label, panel = full_panel_main) {
    if (!outcome_col %in% names(panel)) {
      stop("run_outcome_fullpanel missing required outcome column: ", outcome_col)
    }
    coef_list <- list()
    run_one <- function(sub, spec_label) {
      sub <- sub[!is.na(get(outcome_col))]
      .res <- run_named_fullpanel(sub, spec_label, lhs = outcome_col)
      if (!is.null(.res)) coef_list[[length(coef_list) + 1L]] <<- .res$coefs
    }
    run_one(panel, paste0(out_stem, "_full"))
    for (fb in acq_bins_ordered) {
      .sub <- subset_panel_main(panel,
                                treated_filter = substitute(acq_filer_bin == FB, list(FB = fb)),
                                outcome_col = outcome_col)
      .spec <- paste0(out_stem, "_acq_", acq_bin_suffix(fb))
      run_one(.sub, .spec)
    }
    for (pb in portfolio_bins) {
      .sub <- subset_panel_main(panel,
                                treated_filter = substitute(portfolio_bin == PB, list(PB = pb)),
                                outcome_col = outcome_col)
      .spec <- paste0(out_stem, "_port_", gsub("[^a-z0-9_]", "_", tolower(pb)))
      run_one(.sub, .spec)
    }
    if (length(coef_list) == 0L) return(invisible(NULL))
    ...
    plot_event_coefs(all_coefs, full_spec,
                     ...
                     y_label = outcome_label)
    filer_specs <- grep(paste0("^", out_stem, "_acq_"), unique(all_coefs$spec), value = TRUE)
    if (length(filer_specs) > 1L) {
      plot_event_coefs(all_coefs, filer_specs,
                       paste0("Event Study: ", outcome_label, " by Acquirer Type (", fp$label, ")"),
                       fig_path(paste0(out_stem, "_by_acq_filer.png")),
                       facet = TRUE, y_label = outcome_label)
    }
    port_specs <- grep(paste0("^", out_stem, "_port_"), unique(all_coefs$spec), value = TRUE)
    if (length(port_specs) > 1L) {
      plot_event_coefs(all_coefs, port_specs,
                       paste0("Event Study: ", outcome_label, " by Portfolio Size (", fp$label, ")"),
                       fig_path(paste0(out_stem, "_by_port.png")),
                       facet = TRUE, y_label = outcome_label)
    }
    all_coefs
  }

  run_size_diagnostic_fullpanel <- function(outcome_col, out_stem, outcome_label,
                                            panel = full_panel_main,
                                            weights = ~total_units,
                                            fe = "PID + year") {
    req_cols <- c(outcome_col, "total_units", "PID", "year")
    if (!all(req_cols %in% names(panel))) {
      stop("run_size_diagnostic_fullpanel missing required columns for ", out_stem)
    }
    diag_specs <- list(
      list(label = paste0(out_stem, "_diag_1unit"),
           filter = quote(total_units == 1L),
           pretty = "1-unit buildings"),
      list(label = paste0(out_stem, "_diag_2plus"),
           filter = quote(total_units >= 2L),
           pretty = "2+ unit buildings")
    )
    coef_list <- list()
    for (ds in diag_specs) {
      .sub <- subset_panel_main(panel, sample_filter = ds$filter, outcome_col = outcome_col)
      .res <- run_named_fullpanel(.sub, ds$label, lhs = outcome_col, fe = fe, weights = weights)
      if (!is.null(.res)) coef_list[[length(coef_list) + 1L]] <- .res$coefs
    }
    if (length(coef_list) == 0L) return(invisible(NULL))
    all_coefs <- rbindlist(coef_list, use.names = TRUE, fill = TRUE)
    fwrite(all_coefs, out_path(paste0(out_stem, "_size_diag_coefs.csv")))
    plot_event_coefs(
      all_coefs,
      unique(all_coefs$spec),
      paste0(outcome_label, ": 1-unit vs 2+ Diagnostic (", fp$label, ")"),
      fig_path(paste0(out_stem, "_size_diag.png")),
      facet = TRUE,
      y_label = outcome_label,
      ncol = 2L,
      custom_labels = setNames(
        vapply(diag_specs, `[[`, character(1), "pretty"),
        vapply(diag_specs, `[[`, character(1), "label")
      )
    )
    all_coefs
  }

  run_outcome_tract_fe_fullpanel <- function(outcome_col, out_stem, outcome_label, panel = full_panel_main) {
    req_cols <- c(outcome_col, "tract_year_fe", "PID", "year", "total_units")
    if (!all(req_cols %in% names(panel))) {
      stop("run_outcome_tract_fe_fullpanel missing required columns for ", out_stem)
    }
    .sub <- panel[!is.na(get(outcome_col)) & !is.na(tract_year_fe) & !is.na(total_units) & total_units > 0]
    .res <- run_named_fullpanel(.sub, paste0(out_stem, "_tract_year"),
                                lhs = outcome_col, fe = "PID + tract_year_fe")
    if (is.null(.res)) return(invisible(NULL))
    fwrite(.res$coefs, out_path(paste0(out_stem, "_tract_year_coefs.csv")))
    plot_event_coefs(.res$coefs, unique(.res$coefs$spec),
                     paste0("Event Study: ", outcome_label, " with Tract x Year FE (", fp$label, ")"),
                     fig_path(paste0(out_stem, "_tract_year.png")),
                     y_label = outcome_label)
    .res$coefs
  ...
  run_outcome_tract_fe_by_buyer_fullpanel <- function(outcome_col, out_stem, outcome_label,
                                                       panel = full_panel_main) {
    coef_list <- list()
    for (corp_flag in c(TRUE, FALSE)) {
      .sub <- subset_panel_main(panel,
                                treated_filter = substitute(is_corp == CF, list(CF = corp_flag)),
                                outcome_col = outcome_col)
      .sub <- .sub[!is.na(tract_year_fe)]
      .spec <- paste0(out_stem, "_tract_year_", if (corp_flag) "corporate" else "individual")
      .res <- run_named_fullpanel(.sub, .spec, lhs = outcome_col, fe = "PID + tract_year_fe")
      if (!is.null(.res)) coef_list[[length(coef_list) + 1L]] <- .res$coefs
    }
    if (length(coef_list) == 0L) return(invisible(NULL))
    coefs <- rbindlist(coef_list, use.names = TRUE, fill = TRUE)
  ...
  # --- Rent full-panel override ---
  if ("log_med_rent" %in% names(full_panel_main)) {
    rent_source <- full_panel_main[year >= ANALYSIS_YEAR_MIN & year <= ANALYSIS_YEAR_MAX &
                                     !is.na(total_units) & total_units > 0]
    .tx_cov <- rent_source[!is.na(transfer_year) & !is.na(log_med_rent),
                           .(has_pre = any(event_time < 0L, na.rm = TRUE),
                             has_post = any(event_time >= 0L, na.rm = TRUE)),
                           by = PID]
    .valid_tx_rent <- .tx_cov[has_pre == TRUE & has_post == TRUE, PID]
    .ctrl_cov <- rent_source[is.na(transfer_year) & !is.na(log_med_rent), .(n_obs = .N), by = PID]
    .valid_ctrl_rent <- .ctrl_cov[n_obs >= 2L, PID]
    rent_dt_full <- rent_source[
      (!is.na(transfer_year) & PID %chin% .valid_tx_rent) |
      (is.na(transfer_year) & PID %chin% .valid_ctrl_rent)
    ]
    rent_coef_list <- list()
    .res <- run_named_fullpanel(rent_dt_full[!is.na(log_med_rent)], "rent_full",
                                lhs = "log_med_rent")
    if (!is.null(.res)) rent_coef_list[[length(rent_coef_list) + 1L]] <- .res$coefs
    for (fb in acq_bins_ordered) {
      .sub <- subset_panel_main(rent_dt_full,
                                treated_filter = substitute(acq_filer_bin == FB, list(FB = fb)),
                                outcome_col = "log_med_rent")
      .spec <- paste0("rent_acq_", acq_bin_suffix(fb))
      .res <- run_named_fullpanel(.sub, .spec, lhs = "log_med_rent")
      if (!is.null(.res)) rent_coef_list[[length(rent_coef_list) + 1L]] <- .res$coefs
    }
    for (pb in portfolio_bins) {
      .sub <- subset_panel_main(rent_dt_full,
    ...
    if (length(rent_coef_list) > 0L) {
      all_rent_coefs <- rbindlist(rent_coef_list, use.names = TRUE, fill = TRUE)
      fwrite(all_rent_coefs, out_path("rent_coefs.csv"))
      plot_event_coefs(all_rent_coefs, "rent_full",
                       paste0("Event Study: Log Median Rent (", fp$label, ")"),
                       fig_path("rent_full.png"), y_label = "Log median rent")
      .filer_rent_specs <- grep("^rent_acq_", unique(all_rent_coefs$spec), value = TRUE)
      if (length(.filer_rent_specs) > 1L) {
        plot_event_coefs(all_rent_coefs, .filer_rent_specs,
                         paste0("Event Study: Log Median Rent by Acquirer Type (", fp$label, ")"),
                         fig_path("rent_by_acq_filer.png"), facet = TRUE,
                         y_label = "Log median rent")
    ...
    rent_size_diag <- run_size_diagnostic_fullpanel(
      "log_med_rent",
      "rent",
      "Log median rent",
      panel = rent_dt_full
    )
    rent_robust_list <- list()
    .res <- run_named_fullpanel(rent_dt_full[!is.na(log_med_rent) & !is.na(tract_year_fe)],
                                "rent_tract_year",
                                lhs = "log_med_rent",
                                fe = "PID + tract_year_fe")
    if (!is.null(.res)) rent_robust_list[[length(rent_robust_list) + 1L]] <- .res$coefs
    .valid_ctrl_rent3 <- .ctrl_cov[n_obs >= 3L, PID]
    rent_dt_ctrl3 <- rent_source[
      (!is.na(transfer_year) & PID %chin% .valid_tx_rent) |
      (is.na(transfer_year) & PID %chin% .valid_ctrl_rent3)
    ]
    .res <- run_named_fullpanel(rent_dt_ctrl3[!is.na(log_med_rent)], "rent_ctrl3",
                                lhs = "log_med_rent")
    if (!is.null(.res)) rent_robust_list[[length(rent_robust_list) + 1L]] <- .res$coefs
    .res <- run_named_fullpanel(rent_dt_ctrl3[!is.na(log_med_rent) & !is.na(tract_year_fe)],
                                "rent_ctrl3_tract_year",
                                lhs = "log_med_rent",
                                fe = "PID + tract_year_fe")
    if (!is.null(.res)) rent_robust_list[[length(rent_robust_list) + 1L]] <- .res$coefs
    if (length(rent_robust_list) > 0L) {
      rent_robust_all <- rbindlist(rent_robust_list, use.names = TRUE, fill = TRUE)
      fwrite(rent_robust_all, out_path("rent_robustness_coefs.csv"))
      plot_event_coefs(rent_robust_all, unique(rent_robust_all$spec),
                       paste0("Rent Robustness Checks (", fp$label, ")"),
                       fig_path("rent_robustness.png"),
  ...
  for (rob in list(
    ...
    .co <- run_outcome_fullpanel(rob$col, rob$stem, rob$label)
    if (!is.null(.co)) race_all_coefs[[length(race_all_coefs) + 1L]] <- .co
    .diag <- run_size_diagnostic_fullpanel(rob$col, rob$stem, rob$label)
    if (!is.null(.diag)) race_size_diag_coefs[[length(race_size_diag_coefs) + 1L]] <- .diag
  }
  gender_all_coefs <- list()
  for (rob in list(
    list(col = "infousa_pct_female", stem = "pct_female", label = "Share Female Residents"),
    list(col = "infousa_pct_male", stem = "pct_male", label = "Share Male Residents")
  )) {
    .co <- run_outcome_fullpanel(rob$col, rob$stem, rob$label)
    if (!is.null(.co)) gender_all_coefs[[length(gender_all_coefs) + 1L]] <- .co
  }
  run_outcome_fullpanel("permit_rate", "permit_rate", "Permit Rate (per unit)")
  run_outcome_fullpanel("complaint_rate", "complaint_rate", "Complaint Rate (per unit)")
  run_outcome_fullpanel("occupancy_rate", "occupancy_rate", "Occupancy Rate")
  run_outcome_fullpanel("log_infousa_income", "log_income", "Log Household Income ($1000s)")

  filing_size_diag <- baseline_all[spec %in% c("1unit", "2plus")]
  if (nrow(filing_size_diag) > 0L) {
    fwrite(filing_size_diag, out_path("filing_rate_size_diag_coefs.csv"))
    plot_event_coefs(
      filing_size_diag,
      c("1unit", "2plus"),
      paste0("Filing Rate: 1-unit vs 2+ Diagnostic (", fp$label, ")"),
      fig_path("filing_rate_size_diag.png"),
      facet = TRUE,
      y_label = "Filing Rate (per unit)",
      ncol = 2L,
      custom_labels = c("1unit" = "1-unit buildings", "2plus" = "2+ unit buildings")
    )
  }
  permit_size_diag <- run_size_diagnostic_fullpanel("permit_rate", "permit_rate", "Permit Rate (per unit)")
  occupancy_size_diag <- run_size_diagnostic_fullpanel("occupancy_rate", "occupancy_rate", "Occupancy Rate")
  log_income_size_diag <- run_size_diagnostic_fullpanel("log_infousa_income", "log_income", "Log Household Income ($1000s)")

  if (length(race_all_coefs) > 0L) {
    race_plot_all <- rbindlist(race_all_coefs, use.names = TRUE, fill = TRUE)
    race_full_specs <- intersect(c("pct_asian_full", "pct_black_full", "pct_latino_full",
                                   "pct_white_full", "pct_other_full"),
                                 unique(race_plot_all$spec))
    if (length(race_full_specs) > 1L) {
      plot_event_coefs(race_plot_all, race_full_specs,
                       paste0("Race Shares Around Transfer (", fp$label, ")"),
                       fig_path("race_shares_full.png"),
                       facet = TRUE, y_label = "Share of residents", ncol = 3L,
                       custom_labels = c(
    ...
    race_acq_specs <- intersect(c(
                                ...
                                unique(race_plot_all$spec))
    if (length(race_acq_specs) > 1L) {
      plot_event_coefs(race_plot_all, race_acq_specs,
                       paste0("Race Shares Around Transfer by Acquirer Type (", fp$label, ")"),
                       fig_path("race_shares_by_acq_filer.png"),
                       facet = TRUE, y_label = "Share of residents", ncol = 3L,
                       custom_labels = c(
    ...
    race_port_specs <- intersect(c("pct_asian_port_10_", "pct_black_port_10_",
                                 ...
                                 unique(race_plot_all$spec))
    if (length(race_port_specs) > 1L) {
      plot_event_coefs(race_plot_all, race_port_specs,
                       paste0("Race Shares Around Transfer by Portfolio Size (", fp$label, ")"),
                       fig_path("race_shares_by_port.png"),
                       facet = TRUE, y_label = "Share of residents", ncol = 3L,
                       custom_labels = c(
  ...
  if (length(race_size_diag_coefs) > 0L) {
    race_size_diag <- rbindlist(race_size_diag_coefs, use.names = TRUE, fill = TRUE)
    fwrite(race_size_diag, out_path("race_shares_size_diag_coefs.csv"))
    race_size_diag_specs <- intersect(
      c("pct_asian_diag_1unit", "pct_black_diag_1unit", "pct_latino_diag_1unit",
        "pct_white_diag_1unit", "pct_other_diag_1unit",
        "pct_asian_diag_2plus", "pct_black_diag_2plus", "pct_latino_diag_2plus",
        "pct_white_diag_2plus", "pct_other_diag_2plus"),
      unique(race_size_diag$spec)
    )
    if (length(race_size_diag_specs) > 1L) {
      plot_event_coefs(
        race_size_diag,
        race_size_diag_specs,
        paste0("Race Shares: 1-unit vs 2+ Diagnostic (", fp$label, ")"),
        fig_path("race_shares_size_diag.png"),
        facet = TRUE,
        y_label = "Share of residents",
        ncol = 2L,
        custom_labels = c(
          pct_asian_diag_1unit = "Asian, 1-unit",
          pct_black_diag_1unit = "Black, 1-unit",
          pct_latino_diag_1unit = "Latino, 1-unit",
          pct_white_diag_1unit = "White, 1-unit",
          pct_other_diag_1unit = "Other, 1-unit",
          pct_asian_diag_2plus = "Asian, 2+ units",
          pct_black_diag_2plus = "Black, 2+ units",
          pct_latino_diag_2plus = "Latino, 2+ units",
          pct_white_diag_2plus = "White, 2+ units",
          pct_other_diag_2plus = "Other, 2+ units"
        )
      )
    }
  }

  if (length(gender_all_coefs) > 0L) {
    gender_plot_all <- rbindlist(gender_all_coefs, use.names = TRUE, fill = TRUE)
    gender_full_specs <- intersect(c("pct_female_full", "pct_male_full"), unique(gender_plot_all$spec))
    if (length(gender_full_specs) > 1L) {
      plot_event_coefs(gender_plot_all, gender_full_specs,
                       paste0("Gender Shares Around Transfer (", fp$label, ")"),
                       fig_path("gender_shares_full.png"),
                       facet = TRUE, y_label = "Share of residents", ncol = 2L,
                       custom_labels = c(pct_female_full = "Female", pct_male_full = "Male"))
    }
    gender_acq_specs <- intersect(c(
                                    paste0("pct_female_acq_", acq_bin_suffix(ACQ_BIN_LABELS[1])),
                                  ...
                                  unique(gender_plot_all$spec))
    if (length(gender_acq_specs) > 1L) {
      plot_event_coefs(gender_plot_all, gender_acq_specs,
                       paste0("Gender Shares Around Transfer by Acquirer Type (", fp$label, ")"),
                       fig_path("gender_shares_by_acq_filer.png"),
                       facet = TRUE, y_label = "Share of residents", ncol = 2L,
                       custom_labels = c(
    ...
    gender_port_specs <- intersect(c("pct_female_port_10_", "pct_male_port_10_"),
                                   unique(gender_plot_all$spec))
    if (length(gender_port_specs) > 1L) {
      plot_event_coefs(gender_plot_all, gender_port_specs,
                       paste0("Gender Shares Around Transfer by Portfolio Size (", fp$label, ")"),
                       fig_path("gender_shares_by_port.png"),
                       facet = TRUE, y_label = "Share of residents", ncol = 2L,
                       custom_labels = c(pct_female_port_10_ = "Female", pct_male_port_10_ = "Male"))
  ...
  for (rob in list(
    ...
    .co <- run_outcome_tract_fe_fullpanel(rob$col, rob$stem, rob$label)
    if (!is.null(.co)) tract_fe_coefs[[length(tract_fe_coefs) + 1L]] <- .co
  }
  run_outcome_tract_fe_by_buyer_fullpanel("log_infousa_income", "log_income", "Log Household Income ($1000s)")
  if (length(tract_fe_coefs) > 0L) {
    tract_fe_all <- rbindlist(tract_fe_coefs, use.names = TRUE, fill = TRUE)
    fwrite(tract_fe_all, out_path("tract_year_robustness_coefs.csv"))
    race_tract_specs <- intersect(c("pct_asian_tract_year", "pct_black_tract_year",
                                    "pct_latino_tract_year", "pct_white_tract_year",
                                    "pct_other_tract_year"),
                                  unique(tract_fe_all$spec))
    if (length(race_tract_specs) > 1L) {
      plot_event_coefs(tract_fe_all, race_tract_specs,
                       paste0("Race Shares Around Transfer with Tract x Year FE (", fp$label, ")"),
                       fig_path("race_shares_tract_year.png"),
                       facet = TRUE, y_label = "Share of residents", ncol = 3L,
  ...
  entry_panel <- full_panel_main[year >= max(2011L, ANALYSIS_YEAR_MIN) &
                                   year <= ANALYSIS_YEAR_MAX &
                                   !is.na(total_units) & total_units > 0]
  entry_coefs <- list()
  entry_2plus_rental_license_coefs <- list()
  run_entry_one <- function(sub, outcome_col, spec_label) {
    if (!outcome_col %in% names(sub)) stop("Missing required rental-market column: ", outcome_col)
    sub <- sub[!is.na(get(outcome_col))]
    .res <- run_named_fullpanel(sub, spec_label, lhs = outcome_col)
    if (!is.null(.res)) entry_coefs[[length(entry_coefs) + 1L]] <<- .res$coefs
  }
  if ("entered_license" %in% names(entry_panel)) {
    run_entry_one(entry_panel, "entered_license", "license_entry_full")
    run_entry_one(subset_panel_main(entry_panel, treated_filter = quote(is_corp == TRUE),
                                    outcome_col = "entered_license"),
                  "entered_license", "license_entry_corp")
    run_entry_one(subset_panel_main(entry_panel, treated_filter = quote(is_corp == FALSE),
                                    outcome_col = "entered_license"),
                  "entered_license", "license_entry_indiv")
  }
  if ("entered_altos" %in% names(entry_panel)) {
    run_entry_one(entry_panel, "entered_altos", "altos_entry_full")
  }
  if ("has_rental_evidence" %in% names(entry_panel)) {
    run_entry_one(entry_panel, "has_rental_evidence", "any_rental_entry_full")
    run_entry_one(subset_panel_main(entry_panel, treated_filter = quote(is_corp == TRUE),
                                    outcome_col = "has_rental_evidence"),
                  "has_rental_evidence", "any_rental_entry_corp")
    run_entry_one(subset_panel_main(entry_panel, treated_filter = quote(is_corp == FALSE),
                                    outcome_col = "has_rental_evidence"),
                  "has_rental_evidence", "any_rental_entry_indiv")
  }
  if (length(entry_coefs) > 0L) {
    entry_all <- rbindlist(entry_coefs, use.names = TRUE, fill = TRUE)
    fwrite(entry_all, out_path("rental_entry_coefs.csv"))
    lic_specs <- grep("license_entry", unique(entry_all$spec), value = TRUE)
    if (length(lic_specs) > 0L) {
      plot_event_coefs(entry_all, lic_specs,
                       paste0("Rental License Record Around Transfer (year >= 2011; ref=", fp$ref_period, ")"),
                       fig_path("rental_license_entry.png"),
                       y_label = "Probability of rental license record")
  ...
  if ("entered_license" %in% names(entry_panel)) {
    entry_panel_2plus_rental <- subset_panel_main(
      entry_panel,
      sample_filter = quote(total_units >= 2L & observed_rental_stock == TRUE),
      outcome_col = "entered_license"
    )
    logf("    license_entry_2plus_rental sample: N=", nrow(entry_panel_2plus_rental),
         ", treated PIDs=", entry_panel_2plus_rental[!is.na(transfer_year), uniqueN(PID)],
         ", control PIDs=", entry_panel_2plus_rental[is.na(transfer_year), uniqueN(PID)],
         log_file = log_file)
    for (sp in list(
      list(label = "license_entry_2plus_rental_full", panel = entry_panel_2plus_rental),
      list(label = "license_entry_2plus_rental_corp",
           panel = subset_panel_main(entry_panel_2plus_rental, treated_filter = quote(is_corp == TRUE),
                                     outcome_col = "entered_license")),
      list(label = "license_entry_2plus_rental_indiv",
           panel = subset_panel_main(entry_panel_2plus_rental, treated_filter = quote(is_corp == FALSE),
                                     outcome_col = "entered_license"))
    )) {
      .res <- run_named_fullpanel(sp$panel, sp$label, lhs = "entered_license")
      if (!is.null(.res)) entry_2plus_rental_license_coefs[[length(entry_2plus_rental_license_coefs) + 1L]] <- .res$coefs
    }
    if (length(entry_2plus_rental_license_coefs) > 0L) {
      entry_2plus_license_all <- rbindlist(entry_2plus_rental_license_coefs, use.names = TRUE, fill = TRUE)
      fwrite(entry_2plus_license_all, out_path("rental_entry_2plus_rental_coefs.csv"))
      plot_event_coefs(entry_2plus_license_all, unique(entry_2plus_license_all$spec),
                       paste0("Rental License Record Around Transfer: 2+ Units, Observed Rental Stock (year >= 2011; ref=", fp$ref_period, ")"),
                       fig_path("rental_license_entry_2plus_rental.png"),
                       y_label = "Probability of rental license record")
    }
  }

  # --- Pooled windows ---
  full_panel_pooled <- copy(full_panel_main)
  full_panel_pooled[, window := fcase(
    event_time %in% fp$pre_window, "1_pre",
    event_time %in% fp$transition_window, "2_transition",
    event_time %in% fp$early_post_window, "3_early_post",
    event_time %in% fp$late_post_window, "4_late_post",
    default = NA_character_
  )]
  full_panel_pooled <- full_panel_pooled[!is.na(window) | is.na(transfer_year)]
  pooled_subgroups <- list(
    full = full_panel_pooled,
    corporate = subset_panel_main(full_panel_pooled, treated_filter = quote(is_corp == TRUE), outcome_col = "filing_rate"),
    individual = subset_panel_main(full_panel_pooled, treated_filter = quote(is_corp == FALSE), outcome_col = "filing_rate")
  )
  for (ab in acq_bins_ordered) {
    key <- acq_bin_suffix(ab)
    pooled_subgroups[[key]] <- subset_panel_main(
      full_panel_pooled,
      treated_filter = substitute(acq_filer_bin == AB, list(AB = ab)),
      outcome_col = "filing_rate"
    )
  ...
  pooled_results <- list()
  for (sg_name in names(pooled_subgroups)) {
    sg_dt <- pooled_subgroups[[sg_name]][!is.na(filing_rate)]
    if (nrow(sg_dt) < 100 || sg_dt[, uniqueN(PID)] < 10) next
    fit_pooled <- feols(filing_rate ~ window | PID + year, data = sg_dt,
                        cluster = ~PID, weights = ~total_units)
    ct <- as.data.table(coeftable(fit_pooled), keep.rownames = "term")
    setnames(ct, c("term", "estimate", "std_error", "t_value", "p_value"))
    ct[, subgroup := sg_name]
    ct[, n := fit_pooled$nobs]
    pooled_results[[length(pooled_results) + 1L]] <- ct
  }
  if (length(pooled_results) > 0L) {
    pooled_all <- rbindlist(pooled_results, use.names = TRUE, fill = TRUE)
    fwrite(pooled_all, out_path("pooled_window_coefs.csv"))
    post_pre_summary <- list()
    for (sg_name in unique(pooled_all$subgroup)) {
      sg <- pooled_all[subgroup == sg_name]
      early <- sg[term == "window3_early_post"]
      late <- sg[term == "window4_late_post"]
      if (nrow(early) > 0L && nrow(late) > 0L) {
        avg_post <- (early$estimate + late$estimate) / 2
        avg_se <- sqrt((early$std_error^2 + late$std_error^2) / 4)
        post_pre_summary[[length(post_pre_summary) + 1L]] <- data.table(
          subgroup = sg_name,
          post_minus_pre = round(avg_post, 6),
          se = round(avg_se, 6),
          ci_lower = round(avg_post - 1.96 * avg_se, 6),
    ...
    if (length(post_pre_summary) > 0L) {
      pps_dt <- rbindlist(post_pre_summary, use.names = TRUE, fill = TRUE)
      fwrite(pps_dt, out_path("post_minus_pre_summary.csv"))
      pps_dt[, subgroup_label := clean_spec_labels(subgroup)]
      pps_dt[subgroup_label == subgroup, subgroup_label := subgroup]
      pps_dt[, subgroup_label := factor(subgroup_label, levels = rev(subgroup_label))]
    ...
    key_subgroups <- intersect(c("full", "corporate", "individual",
                                 "highfiler_portfolio_10plus_otherunits", "singlepurchase",
                                 "portfolio_singlepurchase", "portfolio_24",
                                 "portfolio_59", "portfolio_10"),
                               unique(pooled_plot_dt$subgroup))
    if (length(key_subgroups) > 0L) {
      p_pooled <- ggplot(pooled_plot_dt[subgroup %in% key_subgroups],
                         aes(x = window_label, y = estimate, fill = subgroup)) +
        geom_col(position = position_dodge(width = 0.7), width = 0.6) +
        geom_errorbar(aes(ymin = estimate - 1.96 * std_error,
