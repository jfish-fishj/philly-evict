## ============================================================
## retaliatory-evictions-race.R
## ============================================================
## Purpose: Test whether (plausibly) retaliatory eviction filings are
## more likely to be against Black tenants, conditional on tract,
...
read_panel_slice <- function(panel_path, select_cols, pid_filter = NULL) {
  ...
  if (ext == "parquet") {
    ...
    dt <- (arrow::read_parquet(panel_path, col_select = keep))
  } else {
...
fit_cut_heterogeneity <- function(dt, cut_col, sample_name, min_rows = 300L) {
  ...
  for (lv in lvls) {
    ...
    m <- feols(
      case_p_black ~ i(retaliatory_status, ref = "non_retaliatory") |
...
log_file <- p_out(cfg, "logs", "retaliatory-evictions-race.log")

logf("=== Starting retaliatory-evictions-race.R ===", log_file = log_file)
...
for (pp in c(race_case_path, evictions_path, xwalk_path, quarter_panel_path, analytic_path, blp_panel_path)) {
  if (!file.exists(pp)) stop("Missing required input: ", pp)
}

# 1) Race-imputed case outcomes
...
# Drop ambiguous case IDs that map to multiple PID or filing quarter
...
complaints[is.na(lag_severe_complaint_1), lag_severe_complaint_1 := 0L]
complaints[is.na(lead_severe_complaint_1), lead_severe_complaint_1 := 0L]
...
complaints <- complaints[, .(PID, year_quarter, retaliatory_status,retaliatory_severe_status, total_complaints, total_severe_complaints, filed_severe_complaint)]
...
# 5) Final case-level analysis panel
panel <- merge(race_case, case_pid_clean, by = "id", all = FALSE)
...
assert_unique(panel, "id", "retaliatory-race case panel")
...
status_means_full_severe <- panel[, .(
  n_cases = .N,
  mean_case_p_black = mean(case_p_black, na.rm = TRUE),
  mean_case_p_white = mean(case_p_white, na.rm = TRUE)
), by = retaliatory_severe_status][order(retaliatory_severe_status)]

status_means_full_building <- panel[, .(
  n_cases = .N,
  mean_case_p_black = mean(case_p_black, na.rm = TRUE),
  mean_case_p_white = mean(case_p_white, na.rm = TRUE)
), by = .(retaliatory_status, building_type)][order(retaliatory_status, building_type)]

# Descriptives requested: shares by filing-rate cuts, building type, and unit bins.
...
# Main regressions: retaliation status vs race outcomes
m_raw_full <- feols(case_p_black ~retaliatory_status , data = panel[year >= 2015], cluster = ~PID)
m_fe_full <- feols(
...
m_fe_full_building_interact <- feols(
  case_p_black ~ i(retaliatory_severe_status, building_type, ref = "non_retaliatory_severe") |
    tract_geoid + building_type + num_units_bin + year_quarter,
  data = panel[],
  cluster = ~PID
)

m_raw_pre <- feols(case_p_black ~ i(retaliatory_status, ref = "non_retaliatory"), data = panel_pre, cluster = ~PID)
...
m_fe_pre_building_interact <- feols(
  case_p_black ~ i(retaliatory_severe_status,building_type, ref = "non_retaliatory_severe") |
    tract_geoid + building_type + num_units_bin + year_quarter,
  data = panel_pre[n_defendants_total==1],
  cluster = ~PID
)

panel_cls <- panel[case_race_group_l80 %in% c("Black", "White", "Hispanic", "Asian", "Other/Mixed")]
panel_cls[, black_case := as.integer(case_race_group_l80 == "Black")]
panel_cls_pre <- panel_cls[year <= pre_covid_end]
...
effects <- rbindlist(list(
  extract_effects(m_raw_full, "raw_full_case_p_black"),
  extract_effects(m_fe_full, "fe_full_case_p_black"),
  extract_effects(m_raw_pre, "raw_pre_case_p_black"),
  extract_effects(m_fe_pre, "fe_pre_case_p_black"),
  extract_effects(m_bin_fe_full, "fe_full_black_case_logit"),
  extract_effects(m_bin_fe_pre, "fe_pre_black_case_logit")
...
panel_out <- panel[, .(
  id, PID, year, year_quarter, retaliatory_status, retaliatory_severe_status,
...
fwrite(panel_out, file.path(out_dir, "retaliatory_race_case_panel.csv"))
fwrite(status_means_full, file.path(out_dir, "retaliatory_race_status_means.csv"))
fwrite(status_means_full_severe, file.path(out_dir, "retaliatory_race_status_means_severe.csv"))
fwrite(status_means_full_building, file.path(out_dir, "retaliatory_race_status_means_by_building_type.csv"))
fwrite(desc_overall, file.path(out_dir, "retaliatory_race_descriptives_overall.csv"))
fwrite(desc_by_building_type, file.path(out_dir, "retaliatory_race_descriptives_by_building_type.csv"))
fwrite(desc_by_num_units_bin, file.path(out_dir, "retaliatory_race_descriptives_by_num_units_bin.csv"))
fwrite(desc_by_longrun_cut, file.path(out_dir, "retaliatory_race_descriptives_by_longrun_rate_cut.csv"))
fwrite(desc_by_eb_cut, file.path(out_dir, "retaliatory_race_descriptives_by_eb_rate_cut.csv"))
fwrite(desc_by_building_rate_cut, file.path(out_dir, "retaliatory_race_descriptives_by_building_longrun_cut.csv"))
fwrite(effects, file.path(out_dir, "retaliatory_race_effects.csv"))
fwrite(hetero_effects, file.path(out_dir, "retaliatory_race_heterogeneity_rate_cuts.csv"))
writeLines(etable_txt, con = file.path(out_dir, "retaliatory_race_models.txt"))
writeLines(etable_tex, con = file.path(out_dir, "retaliatory_race_models.tex"))
write_latex_table(
  status_means_full,
  path = file.path(out_dir, "retaliatory_race_status_means.tex"),
  digits = 4L,
...
write_latex_table(
  ...
  path = file.path(out_dir, "retaliatory_race_effects.tex"),
  digits = 4L,
  caption = "Retaliatory status coefficient table (race outcomes)"
)
write_latex_table(
  desc_overall,
  path = file.path(out_dir, "retaliatory_race_descriptives_overall.tex"),
  digits = 4L,
  caption = "Retaliatory-status descriptive shares (overall)"
)
write_latex_table(
  desc_by_longrun_cut,
  path = file.path(out_dir, "retaliatory_race_descriptives_by_longrun_rate_cut.tex"),
  digits = 4L,
...
write_latex_table(
  ...
  path = file.path(out_dir, "retaliatory_race_heterogeneity_rate_cuts.tex"),
  digits = 4L,
...
qa_lines <- c(
  ...
  "",
  paste0("Race-case rows (ok, non-missing p_black): ", nrow(race_case)),
  ...
  paste0("Rows in classified-race binary panel: ", nrow(panel_cls)),
  "",
  "Mean race probabilities by retaliatory status (full):",
  ...
  "",
  paste0("Wrote panel: ", file.path(out_dir, "retaliatory_race_case_panel.csv")),
  paste0("Wrote status means: ", file.path(out_dir, "retaliatory_race_status_means.csv")),
  paste0("Wrote status means (severe): ", file.path(out_dir, "retaliatory_race_status_means_severe.csv")),
  paste0("Wrote status means by building: ", file.path(out_dir, "retaliatory_race_status_means_by_building_type.csv")),
  paste0("Wrote status means LaTeX: ", file.path(out_dir, "retaliatory_race_status_means.tex")),
  paste0("Wrote descriptives overall: ", file.path(out_dir, "retaliatory_race_descriptives_overall.csv")),
  paste0("Wrote descriptives by building: ", file.path(out_dir, "retaliatory_race_descriptives_by_building_type.csv")),
  paste0("Wrote descriptives by units bin: ", file.path(out_dir, "retaliatory_race_descriptives_by_num_units_bin.csv")),
  paste0("Wrote descriptives by long-run cut: ", file.path(out_dir, "retaliatory_race_descriptives_by_longrun_rate_cut.csv")),
  paste0("Wrote descriptives by EB cut: ", file.path(out_dir, "retaliatory_race_descriptives_by_eb_rate_cut.csv")),
  paste0("Wrote descriptives by building x long-run cut: ", file.path(out_dir, "retaliatory_race_descriptives_by_building_longrun_cut.csv")),
  paste0("Wrote descriptives overall LaTeX: ", file.path(out_dir, "retaliatory_race_descriptives_overall.tex")),
  paste0("Wrote descriptives by long-run cut LaTeX: ", file.path(out_dir, "retaliatory_race_descriptives_by_longrun_rate_cut.tex")),
  paste0("Wrote effects: ", file.path(out_dir, "retaliatory_race_effects.csv")),
  paste0("Wrote heterogeneity effects: ", file.path(out_dir, "retaliatory_race_heterogeneity_rate_cuts.csv")),
  paste0("Wrote effects LaTeX: ", file.path(out_dir, "retaliatory_race_effects.tex")),
  paste0("Wrote heterogeneity LaTeX: ", file.path(out_dir, "retaliatory_race_heterogeneity_rate_cuts.tex")),
  paste0("Wrote model table: ", file.path(out_dir, "retaliatory_race_models.txt")),
  paste0("Wrote model table LaTeX: ", file.path(out_dir, "retaliatory_race_models.tex"))
)
writeLines(qa_lines, con = file.path(out_dir, "retaliatory_race_qa.txt"))

logf("Wrote outputs to: ", out_dir, log_file = log_file)
logf("=== Finished retaliatory-evictions-race.R ===", log_file = log_file)
