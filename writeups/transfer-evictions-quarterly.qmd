...
# Load all quarterly results (unified output naming: quarterly_ prefix)
baseline_coefs <- fread(file.path(out_dir, "quarterly_baseline_coefs.csv"))
size_coefs     <- fread(file.path(out_dir, "quarterly_size_coefs.csv"))
buyer_coefs    <- fread(file.path(out_dir, "quarterly_buyer_type_coefs.csv"))
raw_means      <- fread(file.path(out_dir, "quarterly_raw_means.csv"))

# Acquirer and portfolio
acq_coefs_path <- file.path(out_dir, "quarterly_acq_filer_coefs.csv")
acq_coefs <- if (file.exists(acq_coefs_path)) fread(acq_coefs_path) else data.table()
acq_desc  <- fread(file.path(out_dir, "quarterly_acq_descriptives.csv"))

port_coefs_path <- file.path(out_dir, "quarterly_portfolio_coefs.csv")
port_coefs <- if (file.exists(port_coefs_path)) fread(port_coefs_path) else data.table()

# Descriptives
desc_buyer <- fread(file.path(out_dir, "quarterly_descriptives.csv"))

# Balance
balance_acq <- fread(file.path(out_dir, "quarterly_balance_by_acq.csv"))
balance_buyer_dt <- fread(file.path(out_dir, "quarterly_balance_by_buyer.csv"))

# QA text for inline stats
qa_lines <- readLines(file.path(out_dir, "quarterly_qa.txt"))

# Extract key stats
med_acq_line <- qa_lines[grep("Median.*filers", qa_lines)]
med_acq <- as.numeric(trimws(gsub(".*: ", "", med_acq_line[1])))
```
...
knitr::include_graphics(file.path(fig_dir, "rtt_quarterly_raw.png"))
```
...
knitr::include_graphics(file.path(fig_dir, "rtt_quarterly_raw_by_size.png"))
```
...
knitr::include_graphics(file.path(fig_dir, "rtt_quarterly_by_size.png"))
```
...
knitr::include_graphics(file.path(fig_dir, "rtt_quarterly_baseline.png"))
```
...
knitr::include_graphics(file.path(fig_dir, "rtt_quarterly_by_buyer.png"))
```
...
```{r acq-stats-table}
kable(acq_desc[, .(acq_filer_bin, n_transfers, n_pids, n_owners,
      ...
      caption = "Acquirer classification (at q=0). Rate = mean pre-acquisition filing rate on acquirer's OTHER properties.") |>
  kable_styling(latex_options = c("hold_position", "scale_down"), font_size = 9)
```
...
knitr::include_graphics(file.path(fig_dir, "rtt_quarterly_by_acq_filer.png"))
```
...
```{r acq-5plus-stats}
# Acquirer descriptives already shown above for full sample
```

```{r acq-5plus-plot, fig.cap="Quarterly event study by acquirer filing behavior, 5+ unit buildings only."}
f5 <- file.path(fig_dir, "rtt_quarterly_by_acq_filer.png")
...
```{r port-stats-table}
# Portfolio descriptives from unified output
```

```{r portfolio-plot, fig.cap="Quarterly event study by portfolio size, 2+ unit buildings."}
knitr::include_graphics(file.path(fig_dir, "rtt_quarterly_by_portfolio.png"))
```
...
f5p <- file.path(fig_dir, "rtt_quarterly_by_portfolio.png")
if (file.exists(f5p)) knitr::include_graphics(f5p)
```

# Acquirer Descriptives {#sec-descriptives}

## Acquirer filing rate descriptives

```{r acq-desc-table}
kable(acq_desc[, .(acq_filer_bin, n_transfers, pct_of_transfers, n_owners,
                    total_units_sum, pct_of_units, pct_corporate, mean_portfolio)],
      col.names = c("Acquirer type", "Transfers", "% of transfers",
                     "Unique owners", "Total units", "% of units",
                     "% corp", "Mean portfolio"),
      digits = c(0, 0, 1, 0, 0, 1, 1, 1),
      format.args = list(big.mark = ","),
      caption = "Acquirer filing rate descriptives. Single-purchase acquirers dominate but portfolio acquirers control a disproportionate share of units.") |>
  kable_styling(latex_options = c("hold_position", "scale_down"), font_size = 9)
```
